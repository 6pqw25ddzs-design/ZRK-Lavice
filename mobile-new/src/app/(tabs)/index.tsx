  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={{ paddingBottom: 32 }}>
      ...
      <View style={s.hero}>
        ...
        <Text style={s.heroTitle}>Razvijamo{'\n'}<Text style={s.heroAccent}>šampionke</Text>{'\n'}budućnosti</Text>
        <TouchableOpacity style={s.btn} onPress={() => router.push('/ekipe')}>
          <Text style={s.btnText}>Naše ekipe →</Text>
        </TouchableOpacity>
      </View>
          ...
          {results.map((r: any) => (
            <View key={r.id} style={s.resultCard}>
              ...
              <Text style={s.resultScore}>
                <Text style={{ color: Colors.primary }}>{r.homeScore}</Text>
                {' : '}
                {r.awayScore}
              </Text>
            </View>
        ...
        <View style={s.section}>
          ...
          {news.map((a: any) => (
            <View key={a.id} style={s.newsCard}>
              ...
              <Text style={s.newsDate}>{new Date(a.publishedAt).toLocaleDateString('sr-ME')}</Text>
            </View>
          ))}
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  heroTitle: { color: Colors.text, fontSize: 32, fontWeight: '800', lineHeight: 40, marginBottom: 20 },
